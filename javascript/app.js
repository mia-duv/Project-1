  ...
  $("#searchBtn").on("click", function (event) {
    ...
    $.ajax(record).then(function (response) {
      ...
      $.ajax(games).then(function (response) {
        // console.log(response.api.games.filter( team => team.hTeam.teamId === selectedTeamId));
        games = response.api.games
        selectedGame = games.filter(
          game => {
            return game.hTeam.teamId === selectedTeamId
          ...
          var col2 = $("<div>");
          var selectedTeamGame = $("<ol>").text(selectedGame);
          col2.append(selectedTeamGame);
          $("#UpGames").append(col2);
